package org.rapid_framework.generator.provider.db.table;
...
import org.rapid_framework.generator.util.XMLHelper.NodeData;
...
//                    columnDefaultValue,
//                    remarks);
            ColumnWeb column = new ColumnWeb(
                    table,
                    sqlType,
                    sqlTypeName,
                    columnName,
    ...
    public List<ColumnWeb> getTableColumns(String table) throws SQLException {
        List primaryKeys = getTablePrimaryKeys(table);
        List<ColumnWeb> columns = new LinkedList<ColumnWeb>();
        ResultSet columnRs = getColumnsResultSet(table);
        ...
        while (columnRs.next()) {
            ...
            String remarks = columnRs.getString("REMARKS");
            if (remarks == null && dbHelper.isOracleDataBase()) {
                remarks = getOracleColumnComments(table, columnName);
            }
...
//            BeanHelper.copyProperties(column, TableOverrideValuesProvider.getColumnOverrideValues(table, column));
    ...
    private ResultSet getColumnsResultSet(String table) throws SQLException {
        ResultSet columnRs = null;
        columnRs = getMetaData().getColumns(getCatalog(), getSchema(), table, null);
        return columnRs;
    }
    ...
    private List<String> getTablePrimaryKeys(String table) throws SQLException {
        // get the primary keys
        List primaryKeys = new LinkedList();
        ResultSet primaryKeyRs = null;
        primaryKeyRs = getMetaData().getPrimaryKeys(getCatalog(), getSchema(), table);
        while (primaryKeyRs.next()) {
            String columnName = primaryKeyRs.getString("COLUMN_NAME");
            GLogger.trace("primary key:" + columnName);
            primaryKeys.add(columnName);
