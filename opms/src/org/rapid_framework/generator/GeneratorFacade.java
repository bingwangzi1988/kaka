...
import org.rapid_framework.generator.provider.db.sql.model.Sql;
...
import org.rapid_framework.generator.provider.db.table.model.Table;
import org.rapid_framework.generator.provider.db.table.model.TableWeb;
import org.rapid_framework.generator.provider.java.model.JavaClass;
import org.rapid_framework.generator.util.BeanHelper;
import org.rapid_framework.generator.util.GLogger;
import org.rapid_framework.generator.util.GeneratorException;
import org.rapid_framework.generator.util.StaticMethodHelper;
...
public class GeneratorFacade {
    ...
    public void generateByTables(String[] tables, HashMap map, String templateRootDir) throws Exception {
        new ProcessUtils().processByTables(tables, map, templateRootDir, false);
    }
    ...
    public class ProcessUtils {
        ...
        public void processByTables(String[] tablenames, HashMap<String, TableWeb> map, String templateRootDir, boolean isDelete) throws Exception {
            List<Table> tables = new ArrayList<Table>();
            for (String tableName : tablenames) {
                Table table = TableFactory.getInstance().getTable(tableName);
                LinkedHashSet<Column> columnList = table.getColumns();
                if (map != null) {
                    TableWeb tableWeb = map.get(tableName);
                    if (tableWeb != null) {
                        BeanHelper.copyProperties(table, tableWeb, new String[]{"columns", "gridPlugins"});

                        List<String> gridPlugins = tableWeb.getGridPlugins();
                        if (gridPlugins != null && !gridPlugins.isEmpty()) {
                            List<String> gridPlugins1 = new ArrayList<String>();
                            for (String gridPlugin : gridPlugins) {
                                gridPlugins1.add(gridPlugin);
                            }
                            table.setGridPlugins(gridPlugins1);
                        }

                        List<ColumnWeb> mapColumnList = tableWeb.getColumns();
                        List<ColumnWeb> columnWebList = new ArrayList<ColumnWeb>(mapColumnList);
                        if (columnList != null && !columnList.isEmpty()) {
                            for (Column column : columnList) {
                                for (Iterator<ColumnWeb> iterator = columnWebList.iterator(); iterator.hasNext(); ) {
                                    ColumnWeb next = iterator.next();
                                    if (column.getSqlName().equals(next.getSqlName())) {
                                        BeanHelper.copyProperties(column, next, new String[]{"_table", "_isIndexed", "_isUnique", "selectCode"});
                                        ((ColumnWeb) column).setSelectCode(next.getSelectCode());
                                        iterator.remove();
                                    }
                                }
    ...
    public static class GeneratorModelUtils {
        ...
        private static HashMap<String, String> getGlobalConfig() {
        ...
        public static void setShareVars(Map templateModel) {
            HashMap globalConfig = getGlobalConfig();
            templateModel.putAll(GeneratorProperties.getProperties());
            templateModel.putAll(System.getProperties());
            templateModel.putAll(globalConfig);
            templateModel.put("env", System.getenv());
            templateModel.put("extjs", StaticMethodHelper.useStaticPackage("org.rapid_framework.generator.genUtil.ExtjsFactory"));
