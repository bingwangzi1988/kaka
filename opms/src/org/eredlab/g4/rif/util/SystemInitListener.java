...
public class SystemInitListener implements ServletContextListener {
	...
	private void systemStartup(ServletContext servletContext) {
		...
		if (forceLoad.equalsIgnoreCase(ArmConstants.FORCELOAD_N)) {
			System.out.println("********************************************");
			System.out.println("海辉金信系统集成与应用开发平台开始启动...");
			System.out.println("********************************************");
		}
		try {
		...
		System.out.println("********************************************");
		if (success) {
			System.out.println("海辉金信系统集成与应用开发平台启动成功[" + G4Utils.getCurrentTime() + "]");
			System.out.println("启动总耗时: " + timeSec / 60 + "分 " + timeSec % 60 + "秒 ");
		} else {
			System.out.println("海辉金信系统集成与应用开发平台启动失败[" + G4Utils.getCurrentTime() + "]");
			System.out.println("启动总耗时: " + timeSec / 60 + "分" + timeSec % 60 + "秒");
		}
		System.out.println("********************************************");
	}
	...
	private void initDbType() throws SQLException {
		...
		if (connection.getMetaData().getDatabaseProductName().toLowerCase().indexOf("ora") > -1) {
			...
			if (log.isErrorEnabled()) {
				log.error(GlobalConstants.Exception_Head + "平台目前还不支持你使用的数据库产品.如需获得支持,请和我们联系!");
			}
			System.exit(0);
		}
	}
